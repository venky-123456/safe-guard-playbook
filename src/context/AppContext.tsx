...
import { User, Incident, ResponseLog, IncidentStatus } from '@/types/cyber';
...
const SEED_INCIDENTS: Incident[] = [
  {
    id: 'INC-001', title: 'Phishing email targeting finance team', description: 'Multiple users in finance received a suspicious email with a fake login page link.', type: 'phishing', severity: 'medium', status: 'investigating', reportedBy: 'analyst', reportedAt: '2026-03-10T09:15:00Z', responseLogs: [
  ...
  {
    id: 'INC-002', title: 'Ransomware detected on workstation', description: 'Malware ransomware encryption detected on endpoint WS-0142.', type: 'malware', severity: 'high', status: 'open', reportedBy: 'analyst', reportedAt: '2026-03-11T14:30:00Z', responseLogs: [],
  },
  {
    id: 'INC-003', title: 'Unauthorized access to admin panel', description: 'Brute force login attempts detected against the admin portal from external IP.', type: 'unauthorized_access', severity: 'high', status: 'resolved', reportedBy: 'admin', reportedAt: '2026-03-09T03:45:00Z', resolvedAt: '2026-03-09T08:12:00Z', responseLogs: [
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const reportIncident = useCallback((title: string, description: string) => {
    ...
    const incident: Incident = {
      id, title, description, type, severity, status: 'open', reportedBy: currentUser?.username || 'unknown', reportedAt: now, responseLogs,
    };
